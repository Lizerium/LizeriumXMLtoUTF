<p align="center">✨Dvurechensky✨</p>

<h1 align="center">🐟 UTF to XML Конвертер 🐟</h1>

<p align="center">
    <img src="https://shields.dvurechensky.pro/badge/Status-In%20Development-yellow">
    <img src="https://shields.dvurechensky.pro/badge/Freelancer-2003-green">
    <img src="https://shields.dvurechensky.pro/badge/C%2B%2B-VS2008-purple">
</p>

<div align="center" style="margin: 20px 0; padding: 10px; background: #1c1917; border-radius: 10px;">
  <strong>🌐 Язык: </strong>
  
  <span style="color: #F5F752; margin: 0 10px;">
    ✅ 🇷🇺 Русский (текущий)
  </span>
  | 
  <a href="./README.md" style="color: #0891b2; margin: 0 10px;">
    🇺🇸 English
  </a>
</div>

---

> [!NOTE]
> Этот проект является частью экосистемы **Lizerium** и относится к направлению:
>
> - [`Lizerium.Tools.Structs`](https://github.com/Lizerium/Lizerium.Tools.Structs)
>
> Если вы ищете связанные инженерные и вспомогательные инструменты, начните оттуда.

## 🐉 Статус

> [!WARNING]
> Проект находится в стадии **разработки**

---

## 🌱 Описание

`UTFXML.exe` — инструмент для запаковки:

- `.cmp`
- `.3db`
- `.mat`
- `.txm`

---

## 🧠 TODO

> [!TIP]
> Планируемые улучшения:

- Учитывать блок `Animations` при запаковке

---

## 🛠 Сборка

> [!IMPORTANT]
> Используется старый toolchain для совместимости

- **IDE:** Visual Studio `2008`
- Режим: `Release`
- Действия:
  - открыть проект
  - собрать
  - готово

---

<p align="center">🐟 Лёгкий инструмент для работы с UTF-ассетами 🐟</p>
